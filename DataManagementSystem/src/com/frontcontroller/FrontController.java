...
public class FrontController extends HttpServlet {
	...
	private void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if(com.equals("/loginCheck.do")) {
			...
			if(ret == 1) viewPage = "authOTP.do"; // if(ret = 1) viewPage = "authOTP.do";
			...
			cmd = new DAddCommand();
			int ret = cmd.execute(request, response);
			if(ret == 1)viewPage = "addOk.jsp";
			else viewPage = "updateFail.jsp";
		}
		else if(com.equals("/deleteData.do")) {
			cmd = new DDeleteCommand();
			int ret = cmd.execute(request, response);
			if(ret == 1)viewPage = "deleteOk.jsp";
			else viewPage = "updateFail.jsp";
		}
		else if (com.equals("/modifyData.do")) {
			cmd = new DModifiyCommand();
			int ret = cmd.execute(request, response);
			if(ret == 1)viewPage = "modifyOk.jsp";
			else viewPage = "updateFail.jsp";
		}
		else {
